...
```
## 四、快速启动
运行项目目录下`run_all.bat`，即可一键启动，浏览器访问`http://localhost:3000/`查看实时监控大屏。
...
大屏采用 2 列 × 3 行网格布局，六个面板：状态仪表盘、故障统计、实时波形、实时频谱、RMS 趋势、历史回放。

刷新频率：状态/波形/频谱 **1 秒**（瞬时状态），RMS 趋势 **3 秒**（需积累），故障统计/历史回放 **5 秒**（累计指标）。
