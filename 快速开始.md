...
| MQTT.fx | 任意版本 | MQTT 客户端调试工具（可选，用于验证连接） |
...
├── src/
│   ├── config.py                # 全局配置参数
│   ├── feature_utils.py         # 特征提取函数 (流水线一/二共用)
│   ├── train_model.py           # 流水线一：离线模型训练
│   ├── producer.py              # 流水线二：MQTT 数据模拟发送
│   ├── consumer.py              # 流水线二：MQTT 接收 + 推理 + 存储
│   └── app.py                   # 流水线二：Flask REST API
├── dashboard/                   # 可视化大屏 (Vue 3 + ECharts)
│   ├── src/App.vue              # 主布局
│   ├── src/components/          # 5 个图表组件
│   └── src/api/index.js         # API 调用封装
├── models/motor_model.pkl       # 训练好的模型文件
├── database/motor_data.db       # SQLite 推理记录
├── outputs/                     # 评估图表输出
...
**执行内容**：数据加载 → 清洗 → 滑动窗口 → 70维特征提取 → 8:2分层划分 → 随机森林训练 → 评估 → 保存模型
...
```cmd
E:
cd E:\BaiduNetdiskDownload\emqx-5.3.2-windows-amd64\bin
.\emqx.cmd console
```
...
#### 终端 3：Producer（MQTT 生产者 — 模拟传感器发送数据）
...
- MQTT.fx（如果开着）的 Subscribe 标签页会实时滚动 JSON 数据
...
| 2 | （可选）MQTT.fx 连接 | 点击 Connect 后指示灯变绿，Subscribe 后能收到消息 |
| 3 | 终端1：`app.py` | `curl http://localhost:5000/api/health` 返回 `{"status":"ok"}` |
| 4 | 终端2：`consumer.py` | 看到 `[MQTT] 已订阅 Topic: motor/sensor_data` |
| 5 | 终端3：`producer.py` | 看到 `[Producer] 开始发送: BF_10HZ.txt...` |
| 6 | 终端4：`npm run dev` | 浏览器 http://localhost:3000 显示大屏，面板有数据 |
...
**现象**：MQTT.fx 点击 Connect 后无法连接，指示灯不绿

**解决**：
1. 确认 EMQX 已正常启动（浏览器能打开 http://localhost:18083）
2. 确认 Broker Address 填写的是 `127.0.0.1`（不是 `localhost`，有些版本 MQTT.fx 不解析域名）
3. 确认 Broker Port 是 `1883`
4. 检查防火墙是否拦截了 1883 端口
